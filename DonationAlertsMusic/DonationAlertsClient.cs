...
public class DonationAlertsClient
{
    ...
    private async Task<string> GetAccessTokenAsync()
    {
        ...
        string authUrl = $"https://www.donationalerts.com/oauth/authorize?client_id={_config.ClientId}&redirect_uri={_config.RedirectUri}&response_type=code&scope=oauth-user-show%20oauth-donation-subscribe";
        ...
        var buffer = Encoding.UTF8.GetBytes("<html><body><script>window.close();</script>Авторизация успешна!</body></html>");
        ...
        var response = await _http.PostAsync("https://www.donationalerts.com/oauth/token", new FormUrlEncodedContent(data));
    ...
    {
        string callback = "jQuery" + new Random().Next(100000, 999999) + "_" + DateTimeOffset.Now.ToUnixTimeMilliseconds();

        string url = $"https://www.donationalerts.com/api/getmediadata?callback={callback}&token={_config.DAWidgetToken}";
        ...
        var mediaItem = mediaArray.FirstOrDefault(m => m["alert_id"]?.Value<string>() == alertId && m["type"]?.Value<string>() == "video");
    ...
    public async Task StartAsync()
    {
        ...
        await _webSocket.ConnectAsync(new Uri("wss://centrifugo.donationalerts.com/connection/websocket"), CancellationToken.None);
        ...
        await SendAsync(new { id = 2, method = 1, @params = new { channel = $"$alerts:donation_{userId}", token = subscribeToken } });
